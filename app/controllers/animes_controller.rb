class AnimesController < ApplicationController
    ...
    def index
        @animes = Anime.all.order("created_at").reverse
    end
    ...
    def scraping
        ...
        doc.css('#mainBlock > div.index_area > div > div > a').each do |anime|
            ...
            @same = Anime.find_by(title: @anime[:title])
            ...
            doc.css('#mainBlock > div.mainEntryBlock > div.mainEntryBase > div.mainEntrykiji > a').each do |doc|
                if @same.nil?
                    ...
                    @same[:ani] = doc[:href].to_s.match(%r{http://www.anitube.se/video.*$}) if !doc[:href].to_s.match(%r{http://www.anitube.se/video.*$}).nil?  #　アニチューブ
                    @same[:hima] = doc[:href].to_s.match(%r{http://himado.in/\d{6}}) if !doc[:href].to_s.match(%r{http://himado.in/\d{6}}).nil? #　ひまわり動画
                    @same[:daily] = doc[:href].to_s.match(%r{http://www.dailymotion.com/video.*$}) if !doc[:href].to_s.match(%r{http://www.dailymotion.com/video.*$}).nil? #　デイリーモーション
                    @same[:miomio] = doc[:href].to_s.match(%r{http://www.miomio.tv/watch.*$}) if !doc[:href].to_s.match(%r{http://www.miomio.tv/watch.*$}).nil? #　MioMio
                    @same[:b9] = doc[:href].to_s.match(%r{http://up.*$}) if !doc[:href].to_s.match(%r{http://up.*$}).nil? #　B9
                    @same[:youtube] = doc[:href].to_s.match(%r{https://www.youtube.com/watch.*$}) if !doc[:href].to_s.match(%r{https://www.youtube.com/watch.*$}).nil? #　Youtube
                    @same[:smove] = doc[:href].to_s.match(%r{http://say-move.org/comeplay.*$}) if !doc[:href].to_s.match(%r{http://say-move.org/comeplay.*$}).nil? #　Saymove
                    @same[:para] = doc[:href].to_s.match(%r{http://parasitestage.com/.*$}) if !doc[:href].to_s.match(%r{http://parasitestage.com/.*$}).nil? #　パラすて
                end
            end
                @same.save! if !@same.nil?
                @anime.save!
        end
          redirect_to root_path
    end
